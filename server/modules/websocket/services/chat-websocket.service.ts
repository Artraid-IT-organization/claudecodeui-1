...
async function handleChatSend(
   ...
   *
   * Егор: «первое, что должно произойти, — отклик, что сообщение принято в
   * работу». Смысл не в вежливости: связь с телефоном рвётся постоянно, и без
   * расписки человек не отличает «запрос не дошёл» от «запрос выполняется, но
   * ответа пока нет». Раньше в этом месте он видел только тишину — и отправлял
   * заново, а повторная отправка ВЫТЕСНЯЛА уже работающий запуск, то есть
   * своими руками убивала то, чего ждала.
   *
   * Отправляем до запуска провайдера: провайдер может думать минуту, а
   * расписка нужна немедленно.
   */
  sendJson(ws, {
    kind: 'chat_accepted',
    sessionId,
