...
/**
 * Сколько сообщений тянуть за один заход.
 *
 * Было 20 — в ленте это около десятка блоков, потому что вызовы инструментов
 * схлопываются в группы. На переписке в две с лишним тысячи сообщений это
 * больше сотни подтягиваний подряд, то есть историю фактически нельзя
 * пролистать.
 *
 * 60 выбрано как компромисс: втрое меньше заходов, но порция всё ещё
 * достаточно мала, чтобы её отрисовка не подвешивала телефон. Поднимать
 * дальше стоит только с замером на реальном устройстве — цена ошибки здесь
 * ровно та, от которой уходим.
 */
